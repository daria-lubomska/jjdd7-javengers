...
public class StatisticsDaoBean {
    ...
    public List<Object[]> getCategoryRank() {
        return entityManager.createNativeQuery("select category.name , count(categories) as "
            + "quantity from RecipeStatistics_categories JOIN category WHERE "
            + "categories = category.id group by categories ORDER BY quantity DESC")
            .getResultList();
    }
