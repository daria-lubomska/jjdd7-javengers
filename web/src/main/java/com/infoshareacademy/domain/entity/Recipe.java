...
@NamedQueries({
        ...
        @NamedQuery(
                ...
                query = "SELECT r FROM Recipe r  JOIN r.ingredients i JOIN r.users u WHERE  u.id=:id "
                    + "AND r.category IN :categories AND r.drinkType IN :drinkTypes AND (i.name "
                    + "IN :ingredients) AND r.isApproved = true GROUP BY r HAVING "
                    + "COUNT(DISTINCT i.name)=:namesLength ORDER BY r.name ASC "),
        @NamedQuery(
                name = "Recipe.findRecipeByCategoryIdAndIngredientNameAndType",
                query = "SELECT r FROM Recipe r  JOIN r.ingredients i WHERE r.category "
                    + "IN :categories  AND r.drinkType IN :drinkTypes AND (i.name IN :ingredients) "
                    + "GROUP BY r HAVING COUNT(DISTINCT i.name)=:namesLength "
        ...
        @NamedQuery(
                ...
                query = "SELECT r FROM Recipe r  WHERE r.category IN :categories "
                    + "AND r.drinkType IN :drinkTypes AND r.isApproved = true ORDER BY r.name ASC"),
        @NamedQuery(
                ...
                query = "SELECT r FROM Recipe r JOIN r.users u WHERE u.id=:id AND r.category "
                    + "IN :categories AND r.drinkType IN :drinkTypes "
                    + "AND r.isApproved = true ORDER BY r.name ASC"),
        @NamedQuery(
