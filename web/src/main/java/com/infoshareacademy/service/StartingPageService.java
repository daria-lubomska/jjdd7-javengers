        ...
        if (checkedOptionList.contains("All Drinks")) {
            ...
            if (checkedIngredientsList.size() == 0 || checkedIngredientsList == null
                ...
                listWithFilters = filteringService
                    .getFiltersQueryByCategoryAndType(parsedToLongCategoriesList, checkedTypesList);
            } else {
                listWithFilters = filteringService
                    .getAllFiltersQuery(parsedToLongCategoriesList, checkedIngredientsList,
                        checkedTypesList);
            }
        } else {
            ...
            if (checkedIngredientsList.size() == 0 || checkedIngredientsList == null
                || checkedIngredientsList.isEmpty()) {
                listWithFilters = filteringService
                    .getFavouritesFiltersQueryByCategoryAndType(parsedToLongCategoriesList,
                        checkedTypesList, userId);
            } else {
                listWithFilters = filteringService
                    .getFavouritesFiltersQuery(parsedToLongCategoriesList, checkedIngredientsList,
                        checkedTypesList, userId);
            }
        }
